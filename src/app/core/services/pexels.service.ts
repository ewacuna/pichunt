...
export class PexelsService {
  ...
  public searchQuery(query: string, page: number): Observable<IPexelsPhotoList> {
    return this.httpClient.get<IPexelsPhotoList>(`search?query=${query}&page=${page}`)
  }
  ...
  public getFeaturedCollections(page?: number, perPage?: number): Observable<IFeaturedCollection> {
    return this.httpClient.get<IFeaturedCollection>(
      `collections/featured?page=${page || 1}&per_page=${perPage || 15}`
    );
  }

  public getCollection(collectionId: string, page?: number, perPage?: number): Observable<IPexelsCollectionData> {
